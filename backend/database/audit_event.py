...
from database.context import current_user_id, skip_audit_logging

__all__ = ["current_user_id", "register_audit_events"]


def _should_audit(target) -> bool:
    if skip_audit_logging.get():
        return False
    if not hasattr(target, "__tablename__") or target.__tablename__ == "audit_logs":
        return False
    return True
...
def after_update_listener(mapper, connection, target):
    if _should_audit(target):
        ...
        old_row = connection.execute(table.select().where(table.c.id == pk)).fetchone()
