...
async def seed_global_modules_and_permissions(
    db: AsyncSession, *, include_tenant: bool = True
) -> None:
    modules = GLOBAL_MODULES if include_tenant else [
        m for m in GLOBAL_MODULES if m[0] != "tenant"
    ]
    permissions = GLOBAL_PERMISSIONS if include_tenant else [
        p for p in GLOBAL_PERMISSIONS if not p[0].startswith("tenant.")
    ]
    ...
    for name, desc in modules:
        ...
        if not module:
            ...
            print(f"Global module {name} already exists")
    ...
    for name, desc in permissions:
        ...
        if not permission:
            ...
            print(f"Global permission {name} already exists")
